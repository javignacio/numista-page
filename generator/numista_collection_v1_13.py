...
def load_date_runs_csv(csv_path: str) -> list[dict]:
...
load_chile_date_runs_csv = load_date_runs_csv
...
UK_PENNY_DATE_RUNS_CSV_PATH = "inputs/uk_penny_date_runs.csv"
...
def render_combined_app(collection_rows: list[dict], wishlist_rows: list[dict], out_html: str, collection_total_items: int, collection_total_types: int, wishlist_total_types: int, chile_date_runs: Optional[List[dict]] = None, uk_penny_date_runs: Optional[List[dict]] = None) -> None:
	...
	uk_penny_date_runs_json = json.dumps(uk_penny_date_runs or [])
...
@media (max-width: 700px){
  ...
  .chileMissingCell{font-weight:600;}
	...
	html.append("<section id='panel-uk-pennies' class='appPanel'><div class='modeHeader compactAnalyticsHeader'><h2>United Kingdom pennies</h2></div><div id='ukPennyRunsPanel' class='analyticsPanel'></div></section>")
	...
	html.append("</div>")
	html.append(f"<script>window.__NUMISTA_ANALYTICS_ROWS = {analytics_rows_json}; window.__NUMISTA_CHILE_DATE_RUNS = {chile_date_runs_json}; window.__NUMISTA_UK_PENNY_DATE_RUNS = {uk_penny_date_runs_json}; window.__NUMISTA_MAP_ROWS = {map_rows_json}; window.__NUMISTA_ALL_COUNTRIES = {all_countries_json};</script>")
...
(function(){
  ...
  function openPanel(panelId){
	...
	if (panelId === 'panel-uk-pennies' && typeof renderUkPennyRuns === 'function') {
	  setTimeout(() => renderUkPennyRuns(), 0);
	}
  ...
  function renderDateRuns(opts){
    ...
    const chileRows = allRows.filter(r => String(r.issuer_root || '').toLowerCase() === opts.issuerRoot);
    ...
    runs.forEach(r => {
      ...
      const currency = (opts.groupBySeries && cr.series) || (ownedRow && ownedRow.currency) || cr.currency || 'Unknown currency';
      if (currency !== lastCurrency){
        const groupCount = runs.filter(x => (((opts.groupBySeries && x.curated.series) || (x.ownedRow && x.ownedRow.currency) || x.curated.currency || 'Unknown currency') === currency)).length;
      ...
      const q = ownedRow ? (ownedRow.km_y ? `ref:${ownedRow.km_y}` : `country:${opts.countryQuery} & ${cr.type_id}`) : `country:${opts.countryQuery}`;
    ...
    const sourceText = usingCurated ? `${opts.label} date checklist: ${runs.length} coin types, ${totalExpected} expected circulation dates.` : `No ${opts.fileLabel} loaded. Using rough range fallback; this can include non-minted years.`;
    ...
    function applyChileRunFilter(){
      const q = normText((panel.querySelector('#chileRunSearch') || {}).value || '');
      const status = (panel.querySelector('#chileRunStatus') || {}).value || 'all';
    ...
    const input = panel.querySelector('#chileRunSearch');
    const statusSel = panel.querySelector('#chileRunStatus');
    ...
    applyChileRunFilter();
    panel.querySelectorAll('.chileRunRow').forEach(row => row.addEventListener('click', () => navigateToMode('collection', [opts.countryFilter], row.dataset.query || `country:${opts.countryQuery}`)));
  }
  function renderChileRuns(){
    renderDateRuns({panelId:'chileRunsPanel', issuerRoot:'chile', curated:window.__NUMISTA_CHILE_DATE_RUNS, label:'Chile', fileLabel:'chile_date_runs.csv', countryQuery:'chile', countryFilter:'Chile', groupBySeries:false});
  }
  function renderUkPennyRuns(){
    renderDateRuns({panelId:'ukPennyRunsPanel', issuerRoot:'united kingdom', curated:window.__NUMISTA_UK_PENNY_DATE_RUNS, label:'UK penny', fileLabel:'uk_penny_date_runs.csv', countryQuery:'united kingdom', countryFilter:'United Kingdom', groupBySeries:true});
  }
...
def main(args):
	...
	uk_penny_date_runs = load_date_runs_csv(args.uk_penny_date_runs_csv)
	if uk_penny_date_runs:
		print(f"Loaded UK penny date-run checklist: {len(uk_penny_date_runs)} types from {args.uk_penny_date_runs_csv}")
	else:
		print(f"No UK penny date-run checklist found at {args.uk_penny_date_runs_csv}; UK pennies tab will use range fallback.")
	...
	render_combined_app(
		...
		uk_penny_date_runs=uk_penny_date_runs,
	)
...
def _parse_args():
	...
	ap.add_argument("--uk-penny-date-runs-csv", default=UK_PENNY_DATE_RUNS_CSV_PATH,
					help="Curated UK penny circulation date-run checklist")
